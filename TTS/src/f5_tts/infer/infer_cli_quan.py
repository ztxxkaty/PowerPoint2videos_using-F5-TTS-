...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="total pages in powerpoint",
)
parser.add_argument(
    "--fix_duration",
    type=float,
...
pages = args.pages if args.pages else config["pages"]
...
def main():
    for page in range(1, pages+1):
        toml_name = f"{curr_path}/{toml_Folder}/{page}.toml"
        if os.path.exists(toml_name):
            ...
            print(f"第{page}页音频已生成\n")
        else:
            print(f"第{page}页没有备注,无法生成对应音频\n")
